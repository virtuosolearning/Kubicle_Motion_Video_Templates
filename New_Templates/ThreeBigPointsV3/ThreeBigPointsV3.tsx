...
// Ports the "3 Big Points v3" prototype:
...
//
// Layout: two side points (left + right). The centre connector is a fixed
// decoration — its link icon is baked into the template, not user-supplied.
...
export const threeBigPointsV3TimingsSchema = z
  .object({
...
export const threeBigPointsV3Schema = z.object({
  // Exactly 2 points — left and right of the centre connector.
  points: z.array(sidePointSchema).length(2),
  timings: threeBigPointsV3TimingsSchema.optional(),
});

export type ThreeBigPointsV3Props = z.infer<typeof threeBigPointsV3Schema>;

export const threeBigPointsV3Meta = {
  description:
    'Two side frames (icon + pill label) flanking a centre connector that scales ' +
    'and rotates into place. BG zooms; sides slide in from off-canvas. Use to set ' +
    'up a relationship or contrast between two parallel concepts.',
  authoringNotes:
    'Always supply exactly 2 points (left, right). icon is an id from the catalog ' +
    '(e.g. "vocabulary", "strong-mind"); the chosen icons should be conceptually ' +
    'parallel since the layout reads as a pairing. label is the pill caption — ' +
    'strict 30-character max, one line at 37 px in Satoshi Bold. Write tight noun ' +
    'phrases or short titles. GOOD: "Word recognition", "Working memory". BAD: ' +
    '"Improving word recognition skills" (too long). Default duration 300 frames (10 s).',
} as const;
...
export const ThreeBigPointsV3: React.FC<ThreeBigPointsV3Props> = ({ points, timings }) => {
  const frame = useCurrentFrame();

  const [handle] = useState(() => delayRender('Loading ThreeBigPointsV3 fonts'));
  useEffect(() => {
    loadFonts()
