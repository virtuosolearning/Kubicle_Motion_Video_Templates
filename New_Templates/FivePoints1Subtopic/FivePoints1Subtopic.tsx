...
// Ports the 5 Points / 1 Subtopic V1 prototype:
...
export const fivePoints1SubtopicTimingsSchema = z
  .object({
...
export const fivePoints1SubtopicSchema = z.object({
  ...
  timings: fivePoints1SubtopicTimingsSchema.optional(),
});

export type FivePoints1SubtopicProps = z.infer<typeof fivePoints1SubtopicSchema>;

export const fivePoints1SubtopicMeta = {
  description:
    'Vertical 5-step roadmap: dark-panel anchor icon on the left; on the right ' +
    'a dotted spine connects 5 milestone circles, each next to a card with title ' +
    'and one-line description. A "spotlight" travels down the spine, lighting up ' +
    'each milestone\'s tick and pulling the active card forward.',
  authoringNotes:
...
export const FivePoints1Subtopic: React.FC<FivePoints1SubtopicProps> = ({
  milestones,
  anchorIcon,
  timings,
}) => {
  ...
  const [handle] = useState(() => delayRender('Loading FivePoints1Subtopic fonts'));
  useEffect(() => {
    loadFonts()
