...
// TreeDiagram4x2V1 — hero panel on the left → 4 caption pills → 2 A/B leaves.
...
//     (same staging idea as ThreeBigPointsV3's BG zoom).
...
export const treeDiagram4x2V1LeafSchema = z.object({
  icon: z.string().min(1),                  // icons/<id>.svg
  text: z.string().min(1).max(34),          // Satoshi Medium 26 px, white
});

export const treeDiagram4x2V1BranchSchema = z.object({
  caption: z.string().min(1).max(22),       // pill label, Satoshi Medium 33 px white
  a: treeDiagram4x2V1LeafSchema,
  b: treeDiagram4x2V1LeafSchema,
});

export const treeDiagram4x2V1TimingsSchema = z
  .object({
...
export const treeDiagram4x2V1Schema = z.object({
  ...
  branches:  z.array(treeDiagram4x2V1BranchSchema).length(4),
  timings:   treeDiagram4x2V1TimingsSchema.optional(),
});

export type TreeDiagram4x2V1Props = z.infer<typeof treeDiagram4x2V1Schema>;

export const treeDiagram4x2V1Meta = {
  description:
  ...
  authoringNotes:
    ...
    'patched SVG. Always supply exactly 4 branches, each with an a and b ' +
    'leaf. caption ≤22 chars (Satoshi Medium 33 px white). leaf text ≤34 ' +
    'chars, kept short to read on one line. Bundled icons: check, x, ' +
    'arrow-up, arrow-down, plus, minus, info, alert-triangle, git-branch, ' +
    'diagram. Default duration 300 frames (10 s).',
} as const;
...
// Sphere gradient — radial highlight top-left — matches Timeline5TilesV2 ball.
const SPHERE_BG =
...
// base. Same palette family as Carousel5TilesV1 tiles.
const OXFORD_BG =
...
export const TreeDiagram4x2V1: React.FC<TreeDiagram4x2V1Props> = ({
  title,
  heroIcon,
  branches,
  timings,
  ...
  const [handle] = useState(() => delayRender('Loading TreeDiagram4x2V1 fonts'));
  useEffect(() => {
    loadFonts()
...
export const treeDiagram4x2V1DefaultProps: TreeDiagram4x2V1Props = {
