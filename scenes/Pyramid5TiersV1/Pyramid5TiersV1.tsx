...
// Pyramid5TiersV1 — left-side pyramid + right-side banner stack.
...
export const pyramid5TiersV1Schema = z.object({
  // 5 tiers, TOP → BOTTOM order. Top tier sits at the pyramid's apex.
  tiers:   z.array(pyramid5TiersV1TierSchema).length(5),
  timings: pyramid5TiersV1TimingsSchema.optional(),
});

export type Pyramid5TiersV1Props = z.infer<typeof pyramid5TiersV1Schema>;

export const pyramid5TiersV1Meta = {
  description:
  ...
  authoringNotes:
    ...
    'shorter on the bottom. icon is the lucide-style SVG id shown inside ' +
    'the slab; bundled set: trophy, monitor, cpu, settings, database. ' +
    'Recommended duration ~13 s (390 frames) to let the full highlight ' +
    'cycle play out — at 10 s only the first ~3 tiers highlight.',
} as const;
...
export const Pyramid5TiersV1: React.FC<Pyramid5TiersV1Props> = ({ tiers, timings }) => {
  const frame = useCurrentFrame();

  const [handle] = useState(() => delayRender('Loading Pyramid5TiersV1 fonts'));
  useEffect(() => {
    loadFonts()
...
export const pyramid5TiersV1DefaultProps: Pyramid5TiersV1Props = {
  tiers: [
    {
      title: 'Outcomes',
