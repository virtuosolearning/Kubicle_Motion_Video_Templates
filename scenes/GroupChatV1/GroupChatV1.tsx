...
// GroupChatV1 — full-bleed desktop chat window in our oxford-blue palette.
...
export const groupChatV1MessageSchema = z.object({
...
export const groupChatV1TimingsSchema = z
  .object({
...
export const groupChatV1Schema = z.object({
  groupName:   z.string().min(1).max(36),
  memberCount: z.number().int().positive(),
  messages:    z.array(groupChatV1MessageSchema).min(3).max(8),
  timings:     groupChatV1TimingsSchema.optional(),
});

export type GroupChatV1Props = z.infer<typeof groupChatV1Schema>;

export const groupChatV1Meta = {
  description:
...
function buildAuthorTints(messages: GroupChatV1Props['messages']): Map<string, string> {
...
function Header({ groupName, memberCount, messages, tints, opacity }: {
  groupName: string;
  memberCount: number;
  messages: GroupChatV1Props['messages'];
...
function MessageRow({
  ...
  message: GroupChatV1Props['messages'][number];
...
export const GroupChatV1: React.FC<GroupChatV1Props> = ({
  groupName,
  memberCount,
  messages,
  timings,
  ...
  const [handle] = useState(() => delayRender('Loading GroupChatV1 fonts'));
  useEffect(() => {
    loadFonts()
...
export const groupChatV1DefaultProps: GroupChatV1Props = {
