...
// Ports the Right Pills + Ticks V1 prototype:
...
export const rightPillsTicksV1TimingsSchema = z
  .object({
...
export const rightPillsTicksV1Schema = z.object({
  ...
  timings: rightPillsTicksV1TimingsSchema.optional(),
});

export type RightPillsTicksV1Props = z.infer<typeof rightPillsTicksV1Schema>;

export const rightPillsTicksV1Meta = {
  description:
    'Hero icon on the left + 5 dark-pill checklist items on the right that ' +
    'fade up, then per-row a white circle slides from the right edge to a ' +
    'left anchor and a tick trim-reveals before the line of text fades in.',
  authoringNotes:
...
export const RightPillsTicksV1: React.FC<RightPillsTicksV1Props> = ({
  responsibilities,
  hero,
  timings,
}) => {
  ...
  const [handle] = useState(() => delayRender('Loading RightPillsTicksV1 fonts'));
  useEffect(() => {
    loadFonts()
...
export const rightPillsTicksV1DefaultProps: RightPillsTicksV1Props = {
  responsibilities: [
...
export const rightPillsTicksV1CharacterDemoProps: RightPillsTicksV1Props = {
  ...rightPillsTicksV1DefaultProps,
