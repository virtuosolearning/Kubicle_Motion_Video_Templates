...
// Flywheel4PetalsV1 — large canvas-filling 4-petal flywheel.
...
export const flywheel4PetalsV1PetalSchema = z.object({
  label: z.string().min(1).max(14),
  body:  z.string().min(1).max(48),
  icon:  z.string().min(1),
});

export const flywheel4PetalsV1TimingsSchema = z
  .object({
...
export const flywheel4PetalsV1Schema = z.object({
  ...
  petals:      z.array(flywheel4PetalsV1PetalSchema).length(4),  // clockwise: top, right, bottom, left
  timings:     flywheel4PetalsV1TimingsSchema.optional(),
});

export type Flywheel4PetalsV1Props = z.infer<typeof flywheel4PetalsV1Schema>;

export const flywheel4PetalsV1Meta = {
  description:
  ...
  authoringNotes:
    ...
    'left]. Each petal has a label (≤14 chars, short phase name) and body ' +
    '(≤48 chars, single supporting line). icon is the lucide-style SVG id ' +
    'shown inside the petal. Default duration 300 frames (10 s).',
} as const;
...
// Carousel5TilesV1 tiles and the TreeDiagram4x2V1 bg).
const HUB_BG =
...
function PetalContent({
  ...
  i: number;
  petal: Flywheel4PetalsV1Props['petals'][number];
  frame: number;
  startF: number;
...
export const Flywheel4PetalsV1: React.FC<Flywheel4PetalsV1Props> = ({
  title,
  subtitle,
  centerIcon,
  petals,
  ...
  const [handle] = useState(() => delayRender('Loading Flywheel4PetalsV1 fonts'));
  useEffect(() => {
    loadFonts()
...
export const flywheel4PetalsV1DefaultProps: Flywheel4PetalsV1Props = {
  title:      'Agentic Loop',
